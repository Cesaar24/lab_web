...
export default defineEventHandler(async (event) => {
  ...
  /* verificar password */
  const _response = await UserMysql.verificacionPasswoord(
    _body?.id,
    _body?.passwordVerification,
  );
  if (!_response.success) {
    throw createError({
      statusMessage: _response.message || "Unauthorized",
      statusCode: _response.statusCode || 401,
    });
  }
  ...
  const response = (await UserMysql.update(newUser)) as {
    success: boolean;
  ...
  if (response.success) {
    ...
    setCookie(event, "access_token", token, {
      httpOnly: true, // Solo accesible a través de HTTPS
    ...
    throw createError({
      ...
      statusMessage: String(response.message) || "Failed to update user",
    });
  }
});
